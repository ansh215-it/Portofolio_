// src/components/About.tsx
import React from 'react';
import AnimatedWrapper from './AnimatedWrapper';

const timelineData = [
  { year: '2022', title: 'Bachelor of Computer Science', subtitle: 'Universitas Diponegoro' },
  { year: '2025', title: 'Web Development Intern', subtitle: 'Diskominfo Semarang' },
//   { year: '2024', title: 'Cloud Computing Cohort', subtitle: 'Bangkit Academy 2024' },
  { year: '2026', title: 'Universitas Diponegoro Graduate', subtitle: 'GPA 3.71/4.00' },
//   { year: '2025 - 2026', title: 'Backend Developer Intern', subtitle: 'PT Balai Pustaka' },
];

const About: React.FC = () => {
  return (
    <section id="about" className="section-container pt-24">
      <AnimatedWrapper>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          
          {/* Kiri: Deskripsi */}
          <div className="space-y-6">
            <h2 className="text-5xl font-extrabold text-textPrimary">About Me</h2>
            <div className="w-full h-px bg-white/10 my-6"></div>
            <p className="text-lg text-textSecondary leading-relaxed">
              I am an Informatics graduate with a keen interest in software development, programming, and design. I have experience developing various software projects and possess strong teamwork, adaptability, and a responsible approach to task completion. I am highly motivated to continuously learn new technologies and hone my skills to deliver effective and innovative solutions.
            </p>
            <div className="flex gap-4 pt-4">
              <button className="px-6 py-3 rounded-full border border-white/20 hover:bg-white/5 transition-colors text-textPrimary font-medium">
                My Resume
              </button>
              {/* <button className="px-6 py-3 rounded-full bg-white text-black hover:bg-gray-200 transition-colors font-medium">
                Get in touch
              </button> */}
            </div>
          </div>

          {/* Kanan: Timeline */}
          <div className="relative border-l border-white/10 pl-8 space-y-12">
            {timelineData.map((item, index) => (
              <div key={index} className="relative">
                {/* Dot Timeline */}
                <div className="absolute -left-[37px] top-1.5 w-3 h-3 bg-white rounded-full"></div>
                <div className="flex flex-col md:flex-row md:items-baseline gap-2 md:gap-6">
                  <span className="text-textSecondary text-sm font-mono min-w-[80px]">{item.year}</span>
                  <div>
                    <h3 className="text-xl font-bold text-textPrimary">{item.title}</h3>
                    <p className="text-textSecondary">{item.subtitle}</p>
                  </div>
                </div>
              </div>
            ))}
          </div>

        </div>
      </AnimatedWrapper>
    </section>
  );
};

export default About;